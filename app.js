...
function exportTunggakanPDF() {
  ...
  const { sppList, catList, namaBulan } = laporanView.data;
  const tab = laporanView.tab;
  const tahunAjaran = state.settings.tahunAjaran || '';

  // ── Tentukan data & label tab ──────────────────────────────
  let filtered = [];
  let labelTab = '';
  if (tab === 'semua') {
    filtered = [
  ...
  const tglCetak    = new Date().toLocaleDateString('id-ID', { weekday:'long', day:'2-digit', month:'long', year:'numeric' });
  const totalPiutang = filtered.reduce((s, x) => s + x.nominal, 0);

  // ── Baris tabel — bin/binti kolom terpisah ─────────────────
  const rows = filtered.map((x, i) => {
    const binLabel = x.gender === 'santriwati' ? 'binti' : 'bin';
    const binCell  = x.bin ? `${binLabel} ${x.bin}` : '-';
    const jenisBg  = x.jenisLabel === 'SPP'
      ? 'background:#dbeafe;color:#1e40af'
      : 'background:#fef3c7;color:#92400e';
    return `
    <tr>
      <td class="center">${i + 1}</td>
      <td class="bold">${x.nama}</td>
      <td>${binCell}</td>
      <td>${x.sub}</td>
      <td class="center"><span style="padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;${jenisBg}">${x.jenisLabel}</span></td>
      <td class="right red">Rp ${formatRp(x.nominal)}</td>
    </tr>`;
  }).join('');

  // ── Ringkasan per jenis ─────────────────────────────────────
  const jmlSpp = filtered.filter(x => x.jenisLabel === 'SPP').length;
  const jmlCat = filtered.filter(x => x.jenisLabel === 'Catering').length;
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  body { font-family:'Times New Roman',Times,serif; font-size:12pt; color:#111; padding:2cm 2.5cm; }

  /* ── KOP SURAT ── */
  .kop { display:flex; align-items:center; border-bottom:3px double #1a3a6e; padding-bottom:10px; margin-bottom:4px; }
  .kop-logo { width:70px; height:70px; margin-right:16px; object-fit:contain; flex-shrink:0; }
  .kop-logo-placeholder { width:70px; height:70px; border:2px solid #1a3a6e; border-radius:8px; margin-right:16px;
    display:flex; align-items:center; justify-content:center; font-size:9px; color:#1a3a6e; text-align:center; flex-shrink:0; }
  .kop-text { text-align:center; flex:1; }
  .kop-text .nama-pesantren { font-size:18pt; font-weight:900; color:#1a3a6e; letter-spacing:.5px; text-transform:uppercase; }
  .kop-text .alamat { font-size:9pt; color:#444; margin-top:3px; line-height:1.5; }
  .kop-sub { text-align:center; font-size:8.5pt; color:#666; margin-bottom:12px; border-bottom:1px solid #ccc; padding-bottom:6px; }

  /* ── JUDUL SURAT ── */
  .judul { text-align:center; margin:18px 0 6px; }
  .judul h1 { font-size:14pt; font-weight:900; text-transform:uppercase; letter-spacing:.5px; text-decoration:underline; }
  .judul .sub { font-size:11pt; margin-top:4px; }

  /* ── META INFO ── */
  .meta-tabel { width:100%; margin:14px 0; font-size:10.5pt; border-collapse:collapse; }
  .meta-tabel td { padding:2px 6px; vertical-align:top; }
  .meta-tabel td:first-child { width:160px; font-weight:600; }
  .meta-tabel td:nth-child(2) { width:10px; }

  /* ── PEMBUKA ── */
  .pembuka { font-size:11pt; margin-bottom:12px; line-height:1.7; }

  /* ── TABEL DATA ── */
  table.data { width:100%; border-collapse:collapse; margin:12px 0; font-size:10.5pt; }
  table.data thead tr { background:#1a3a6e; color:#fff; }
  table.data th { padding:7px 9px; font-weight:700; border:1px solid #1a3a6e; }
  table.data td { padding:6px 9px; border:1px solid #ccc; vertical-align:middle; }
  table.data tbody tr:nth-child(even) td { background:#f5f7fa; }
  table.data .total td { background:#fef2f2; font-weight:700; color:#b91c1c; border-top:2px solid #b91c1c; }
  ...
  /* ── RINGKASAN ── */
  .ringkasan { margin:10px 0 18px; font-size:10.5pt; }
  .ringkasan td { padding:2px 8px; }

  /* ── PENUTUP & TTD ── */
  .penutup { font-size:11pt; line-height:1.8; margin-bottom:24px; }
  .ttd-wrap { display:flex; justify-content:flex-end; margin-top:28px; }
  .ttd { text-align:center; font-size:11pt; }
  .ttd .ttd-nama { font-weight:700; text-decoration:underline; margin-top:64px; }
  .ttd .ttd-jabatan { font-size:10pt; color:#444; }

  /* ── FOOTER ── */
  .footer-doc { text-align:center; font-size:9pt; color:#999; border-top:1px solid #ddd; margin-top:32px; padding-top:8px; }

  @media print {
    body { padding:1.5cm 2cm; }
    .kop-logo-placeholder svg { display:none; }
  }
</style>
</head><body>
...
<!-- KOP SURAT -->
<div class="kop">
  <div class="kop-logo-placeholder">LOGO<br>PESANTREN</div>
  <div class="kop-text">
    <div class="nama-pesantren">Pondok Pesantren Darul Falah</div>
    <div class="alamat">
      Jl. Ahmad Yani KM.15, Gambut, Kabupaten Banjar, Kalimantan Selatan 70652<br>
      Telp. — &nbsp;|&nbsp; Email: —
    </div>
  </div>
</div>
<div class="kop-sub">Lembaga Pendidikan Islam · Tahun Ajaran ${tahunAjaran}</div>

<!-- JUDUL SURAT -->
<div class="judul">
  <h1>Laporan Tunggakan ${labelTab}</h1>
  <div class="sub">Bulan <strong>${namaBulan}</strong></div>
</div>

<!-- META INFORMASI -->
<table class="meta-tabel">
  <tr><td>Jumlah Tunggakan</td><td>:</td><td><strong>${filtered.length} data</strong>${jmlSpp && jmlCat ? ` (SPP: ${jmlSpp}, Catering: ${jmlCat})` : ''}</td></tr>
  <tr><td>Total Piutang</td><td>:</td><td><strong style="color:#b91c1c">Rp ${formatRp(totalPiutang)}</strong></td></tr>
  <tr><td>Tanggal Cetak</td><td>:</td><td>${tglCetak}</td></tr>
</table>

<!-- KALIMAT PEMBUKA -->
<div class="pembuka">
  Berikut ini adalah daftar santri/santriwati Pondok Pesantren Darul Falah Gambut yang
  tercatat <strong>belum melunasi pembayaran ${labelTab}</strong> pada bulan <strong>${namaBulan}</strong>.
  Mohon perhatian dan tindak lanjut dari pihak terkait.
</div>
...
<table class="data">
  ...
  <tbody>
    ...
    <tr class="total">
      ...
      <td class="right">Rp ${formatRp(totalPiutang)}</td>
    </tr>
  </tbody>
</table>
...
<!-- KALIMAT PENUTUP -->
<div class="penutup">
  Demikian laporan ini dibuat berdasarkan data yang tercatat dalam sistem manajemen
  Pondok Pesantren Darul Falah Gambut. Apabila terdapat kekeliruan data, mohon segera
  menghubungi bagian administrasi.
</div>

<!-- TANDA TANGAN -->
<div class="ttd-wrap">
  <div class="ttd">
    <div>Gambut, ${tglCetak}</div>
    <div>Bendahara Pesantren,</div>
    <div class="ttd-nama">(.................................)</div>
    <div class="ttd-jabatan">Bendahara PP. Darul Falah Gambut</div>
  </div>
</div>

<!-- FOOTER -->
<div class="footer-doc">Dicetak otomatis dari Sistem Manajemen PP. Darul Falah Gambut &mdash; ${tglCetak}</div>
